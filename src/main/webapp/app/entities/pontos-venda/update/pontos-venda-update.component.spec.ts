...
describe('PontosVenda Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should call tiposPontosVenda query and add missing value', () => {
      const pontosVenda: IPontosVenda = { id: 456 };
      const tiposPontosVenda: ITiposPontoVenda = { id: 12028 };
      pontosVenda.tiposPontosVenda = tiposPontosVenda;

      const tiposPontosVendaCollection: ITiposPontoVenda[] = [{ id: 29403 }];
      jest.spyOn(tiposPontoVendaService, 'query').mockReturnValue(of(new HttpResponse({ body: tiposPontosVendaCollection })));
      const expectedCollection: ITiposPontoVenda[] = [tiposPontosVenda, ...tiposPontosVendaCollection];
      jest.spyOn(tiposPontoVendaService, 'addTiposPontoVendaToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ pontosVenda });
      comp.ngOnInit();

      expect(tiposPontoVendaService.query).toHaveBeenCalled();
      expect(tiposPontoVendaService.addTiposPontoVendaToCollectionIfMissing).toHaveBeenCalledWith(
        tiposPontosVendaCollection,
        tiposPontosVenda
      );
      expect(comp.tiposPontosVendasCollection).toEqual(expectedCollection);
    });
    ...
    it('Should update editForm', () => {
      ...
      pontosVenda.pontosCardeais = pontosCardeais;
      const tiposPontosVenda: ITiposPontoVenda = { id: 52706 };
      pontosVenda.tiposPontosVenda = tiposPontosVenda;
      ...
      expect(comp.tiposPontosVendasCollection).toContain(tiposPontosVenda);
  ...
  describe('Tracking relationships identifiers', () => {
    ...
    describe('trackTiposPontoVendaById', () => {
      it('Should return tracked TiposPontoVenda primary key', () => {
        const entity = { id: 123 };
        const trackResult = comp.trackTiposPontoVendaById(0, entity);
        expect(trackResult).toEqual(entity.id);
      });
    });
